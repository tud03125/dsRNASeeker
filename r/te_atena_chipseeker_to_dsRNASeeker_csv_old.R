...
candidate_mode   <- tolower(get_arg("--candidate-mode", "strict"))
candidate_min_mean <- as.numeric(get_arg("--candidate-min-mean", "10"))
candidate_padj_max <- as.numeric(get_arg("--candidate-padj-max", "1"))
candidate_lfc_threshold <- as.numeric(get_arg("--candidate-lfc-threshold", "0"))
...
if (!candidate_mode %in% c("strict", "expressed")) {
  stop("--candidate-mode must be 'strict' or 'expressed'; got: ", candidate_mode)
}
...
# For custom assemblies, ChIPseeker is intentionally called with annoDb=NULL:
# a standard OrgDb can fail on strain-specific identifiers. Build a conservative
# gene_id -> SYMBOL table from the exact GTF and use org.Mm.eg.db only as a
# fallback for standard Ensembl mouse IDs.
custom_gtf_gene_map <- NULL

map_gene_ids_with_orgdb <- function(gene_ids, orgdb_package) {
  gene_ids <- as.character(gene_ids)
  cleaned <- sub("\\.[0-9]+$", "", gene_ids)
  out <- rep(NA_character_, length(gene_ids))

  if (length(gene_ids) == 0L) return(out)

  orgdb_obj <- tryCatch(
    get(orgdb_package, envir=asNamespace(orgdb_package)),
    error=function(e) NULL
  )
  if (is.null(orgdb_obj)) return(out)

  available_keytypes <- tryCatch(
    AnnotationDbi::keytypes(orgdb_obj),
    error=function(e) character()
  )

  keytype <- NULL
  nonempty <- cleaned[!is.na(cleaned) & nzchar(cleaned)]
  if (
    length(nonempty) > 0L &&
    mean(grepl("^ENSMUSG[0-9]+$", nonempty)) >= 0.5 &&
    "ENSEMBL" %in% available_keytypes
  ) {
    keytype <- "ENSEMBL"
  } else if (
    length(nonempty) > 0L &&
    mean(grepl("^[0-9]+$", nonempty)) >= 0.5 &&
    "ENTREZID" %in% available_keytypes
  ) {
    keytype <- "ENTREZID"
  }

  if (is.null(keytype)) return(out)

  unique_keys <- unique(nonempty)
  mapped <- tryCatch(
    ...
    error=function(e) {
      message(
        "OrgDb fallback symbol mapping failed for keytype ", keytype,
        ": ", conditionMessage(e)
      )
      NULL
    }
  ...
  if (is.null(mapped)) return(out)
  out <- unname(mapped[cleaned])
  as.character(out)
}

if (!is.null(txdb_gtf) && nzchar(txdb_gtf)) {
  if (!requireNamespace("rtracklayer", quietly=TRUE)) {
    stop("Custom-GTF symbol mapping requires the rtracklayer package.")
  }

  message("Reading custom GTF gene identifier mapping: ", txdb_gtf)
  gtf_for_symbols <- rtracklayer::import(txdb_gtf)
  gtf_meta <- as.data.frame(
    S4Vectors::mcols(gtf_for_symbols),
    stringsAsFactors=FALSE
  )

  message(
    "Custom GTF metadata columns: ",
    paste(colnames(gtf_meta), collapse=", ")
  )

  gene_id_col <- intersect(
    c(
      "gene_id", "geneId", "gene", "ID",
  ...
  gene_name_col <- intersect(
    c(
      "gene_name", "geneName", "Name", "gene_symbol",
      "symbol", "external_gene_name"
    ),
    colnames(gtf_meta)
  )

  if (length(gene_id_col) > 0L) {
    gene_ids <- as.character(gtf_meta[[gene_id_col[[1]]]])
    gene_symbols <- rep(NA_character_, length(gene_ids))

    if (length(gene_name_col) > 0L) {
      gene_symbols <- as.character(gtf_meta[[gene_name_col[[1]]]])
    } else {
      message(
        "Custom GTF has gene IDs but no explicit gene-name attribute; ",
        "trying OrgDb fallback mapping."
      )
    }

    fill <- is.na(gene_symbols) | !nzchar(gene_symbols)
    if (any(fill)) {
      orgdb_symbols <- map_gene_ids_with_orgdb(
        gene_ids,
        orgdb_package
      )
      gene_symbols[fill] <- orgdb_symbols[fill]
    }

    custom_gtf_gene_map <- data.frame(
      geneId=gene_ids,
      SYMBOL=gene_symbols,
      stringsAsFactors=FALSE
    )
  } else {
    # A TxDb still retains gene IDs even when rtracklayer did not expose a
    # recognizable GTF metadata column. Use those IDs for an OrgDb fallback.
    txdb_genes <- tryCatch(
      GenomicFeatures::genes(txdb),
      error=function(e) NULL
    )
    gene_ids <- if (is.null(txdb_genes)) {
      character()
    } else {
      names(txdb_genes)
    }
    ...
    custom_gtf_gene_map <- data.frame(
      geneId=gene_ids,
      SYMBOL=map_gene_ids_with_orgdb(gene_ids, orgdb_package),
      stringsAsFactors=FALSE
    )
    message(
      "Custom GTF did not expose a recognized gene_id metadata column; ",
      "trying TxDb gene IDs with OrgDb fallback."
    )
  }

  custom_gtf_gene_map <- custom_gtf_gene_map[
    !is.na(custom_gtf_gene_map$geneId) &
    nzchar(custom_gtf_gene_map$geneId) &
    !is.na(custom_gtf_gene_map$SYMBOL) &
    nzchar(custom_gtf_gene_map$SYMBOL),
    ,
    drop=FALSE
  ]
  custom_gtf_gene_map <- custom_gtf_gene_map[
    !duplicated(custom_gtf_gene_map$geneId),
    ,
    drop=FALSE
  ]

  message(
    "Usable custom GTF/OrgDb gene-symbol mappings: ",
    nrow(custom_gtf_gene_map)
  )

  rm(gtf_for_symbols, gtf_meta)
  invisible(gc())
}
...
# Candidate universe for dsRNASeeker.
# strict    = historical behavior: padj < alpha and |log2FC| > lfc_threshold.
# expressed = validation/benchmarking behavior: expressed/observed TEs enter
#             pair construction; differential significance is kept as evidence.
build_candidate_table <- function(de_df) {
  x <- de_df %>%
    mutate(
      TE_mean_max = pmax(mean_group1, mean_group2, na.rm=TRUE),
      TE_strict_sig = !is.na(padj) & padj < alpha & !is.na(log2FoldChange) & abs(log2FoldChange) > lfc_threshold,
      TE_case_up_strict = !is.na(padj) & padj < alpha & !is.na(log2FoldChange) & log2FoldChange > lfc_threshold,
      TE_control_up_strict = !is.na(padj) & padj < alpha & !is.na(log2FoldChange) & log2FoldChange < -lfc_threshold,
      TE_candidate_mode = candidate_mode
    )

  if (candidate_mode == "strict") {
    x <- x %>% filter(TE_strict_sig)
  } else {
    x <- x %>%
      filter(
        !is.na(TE_mean_max),
        TE_mean_max >= candidate_min_mean,
        !is.na(log2FoldChange),
        abs(log2FoldChange) >= candidate_lfc_threshold
      )
    if (is.finite(candidate_padj_max) && candidate_padj_max < 1) {
      x <- x %>% filter(!is.na(padj), padj < candidate_padj_max)
    }
  }
...
candidate_all <- build_candidate_table(de_out$shrunk)
...
rmsk_ann_candidate <- extract_rmsk_annotation(rmsk, candidate_all$TE_id)

candidate_annot <- candidate_all %>%
  left_join(rmsk_ann_candidate, by="TE_id") %>%
  arrange(desc(TE_strict_sig), desc(abs(log2FoldChange)), desc(TE_mean_max))
...
# ChIPseeker's data-frame column classes can depend on the TxDb and seqname
...
normalize_peak_annotation_types <- function(x) {
  ...
  character_cols <- c(
    "seqnames", "strand", "TE_id", "name", "peak", "V4",
    "annotation", "geneChr", "geneStrand", "geneId",
    "transcriptId", "SYMBOL", "GENENAME", "ENSEMBL"
  )
  for (col in intersect(character_cols, colnames(x))) {
    x[[col]] <- as.character(x[[col]])
  }

  numeric_cols <- c(
    "start", "end", "width", "geneStart", "geneEnd",
    "geneLength", "distanceToTSS"
  )
  for (col in intersect(numeric_cols, colnames(x))) {
...
annotate_te_regions <- function(te_table, out_prefix, txdb, annoDb,
  ...
  # ChIPseeker is called with annoDb=NULL for custom assemblies because a
  # standard OrgDb may not recognize strain-specific IDs. Backfill symbols from
  # the exact GTF used to build this TxDb.
  if (
    !is.null(custom_gtf_gene_map) &&
    nrow(custom_gtf_gene_map) > 0L &&
    "geneId" %in% colnames(supported_df)
  ) {
    supported_df$geneId <- as.character(supported_df$geneId)
    symbol_lookup <- stats::setNames(
      custom_gtf_gene_map$SYMBOL,
      custom_gtf_gene_map$geneId
    )
    mapped_symbols <- unname(symbol_lookup[supported_df$geneId])

    if (!"SYMBOL" %in% colnames(supported_df)) {
      supported_df$SYMBOL <- NA_character_
    }
    existing_symbol <- as.character(supported_df$SYMBOL)
    fill_symbol <- is.na(existing_symbol) | !nzchar(existing_symbol)
    existing_symbol[fill_symbol] <- mapped_symbols[fill_symbol]
    supported_df$SYMBOL <- existing_symbol

    if (!"GENENAME" %in% colnames(supported_df)) {
      supported_df$GENENAME <- supported_df$SYMBOL
    } else {
      existing_name <- as.character(supported_df$GENENAME)
      fill_name <- is.na(existing_name) | !nzchar(existing_name)
      existing_name[fill_name] <- supported_df$SYMBOL[fill_name]
      supported_df$GENENAME <- existing_name
    }

    if (!"ENSEMBL" %in% colnames(supported_df)) {
      supported_df$ENSEMBL <- supported_df$geneId
    }
  }

  supported_df <- normalize_peak_annotation_types(supported_df)
  unsupported_df <- normalize_peak_annotation_types(unsupported_df)

  # Bind only after schema normalization. This is required for custom/T2T
  # assemblies where ChIPseeker may return geneChr or geneStrand as integer,
  # while unsupported-contig placeholders necessarily begin as typed NA.
...
candidate_peak_anno <- annotate_te_regions(
  candidate_annot,
  paste0(comparison, "_", candidate_mode, "_candidates"),
  txdb,
  annoDb,
  custom_genome=is_custom_genome
)
...
merged_candidates <- safe_merge_peak(
  candidate_annot,
  candidate_peak_anno,
  file.path(outdir, "annotation", paste0("TE_expression_annotation_", comparison, "_", candidate_mode, "_candidates.csv"))
)

strict_dsRNASeeker_csv <- file.path(
  outdir, "annotation", paste0("TE_expression_annotation_", comparison, "_all_sig.dsRNASeeker.csv")
)
expressed_dsRNASeeker_csv <- file.path(
  outdir, "annotation", paste0("TE_expression_annotation_", comparison, "_expressed_candidates.dsRNASeeker.csv")
)

write.csv(merged_all, strict_dsRNASeeker_csv, row.names=FALSE)
write.csv(merged_candidates, expressed_dsRNASeeker_csv, row.names=FALSE)

# dsRNASeeker-facing canonical CSV. This is the file consumed by the Python pipeline.
if (candidate_mode == "expressed") {
  write.csv(merged_candidates, out_file, row.names=FALSE)
} else {
...
extract_introns(merged_candidates, candidate_peak_anno, file.path(outdir, "annotation", paste0("intron_TE_", comparison, "_", candidate_mode, "_candidates.csv")))

summary_df <- tibble(
  metric = c("n_total_TE_features", "n_tested_after_count_filter", "n_sig_all_directions", "n_sig_up_only", "candidate_mode", "candidate_min_mean", "candidate_padj_max", "candidate_lfc_threshold", "n_candidate_universe"),
  value = c(nrow(assay(emq)), nrow(de_out$shrunk), nrow(sig_all_annot), nrow(sig_up_annot), candidate_mode, candidate_min_mean, candidate_padj_max, candidate_lfc_threshold, nrow(candidate_annot))
)
...
message("Strict dsRNASeeker TE CSV: ", strict_dsRNASeeker_csv)
message("Expressed-candidate dsRNASeeker TE CSV: ", expressed_dsRNASeeker_csv)
message("Canonical dsRNASeeker TE CSV used by this run: ", out_file)
